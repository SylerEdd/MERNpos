...
import { getAllMenuItems, createMenuItem, updateMenuItem, deleteMenuItemById } from "../../api/menuItemApi";
import { Trash2, Plus, Pencil, Check, X} from "lucide-react";
import { SectionType } from "../../../../pos-backend/src/enums/SectionType"
...
export function MenuManager() {
    ...
    const [newSection, setNewSection] = useState<SectionType>(SectionType.FOOD);
    ...
    const handleAdd = async () => {
        ...
        const isDuplicate = items.some((item) => item.name.toLowerCase() === newName.trim().toLowerCase());
        if (isDuplicate) {
        ...
        try {
            ...
            setNewName("");
            setNewPrice("");
            setNewSection(SectionType.FOOD);
            setShowForm(false);
            setFormError("");
            fetchItems(); 
        } catch (err) {
        ...
        <div className="mb-6 flex items-center gap-3 bg-white p-4 rounded-xl shadow-sm">
            <div className="flex items-center gap-3">
                ...
                <select
                    ...
                    className="border border-gray-300 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0C2B4E]"
                >
                    {Object.values(SectionType).map((s) => (
